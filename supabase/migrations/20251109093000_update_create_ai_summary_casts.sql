...
-- Migration: Cast enum values in create_ai_summary to fix type mismatch
...
BEGIN
    ...
    INSERT INTO assessment_summaries (
        ...
        p_ai_summary,
        'ai_generated'::summary_type,
        'pending_approval'::summary_approval_status,
        1,
        NOW(),
        p_student_user_id
    )
    ...
    DO UPDATE SET
        ...
        summary_type = CASE
            ...
            ELSE 'ai_generated'::summary_type
        END,
        approval_status = CASE
            WHEN assessment_summaries.approval_status = 'approved'::summary_approval_status
                 AND assessment_summaries.ai_summary IS NOT DISTINCT FROM EXCLUDED.ai_summary
            THEN 'approved'::summary_approval_status
            ELSE 'pending_approval'::summary_approval_status
        END,
        version = CASE
            WHEN assessment_summaries.ai_summary IS NOT DISTINCT FROM EXCLUDED.ai_summary
