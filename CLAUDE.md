...
- **Cleanup caveat**: MSG91 defines all four globals (`initSendOTP`, `sendOtp`, `verifyOtp`, `retryOtp`) as **non-configurable** on `window`. In strict mode (all ES-module bundles) `delete window.sendOtp` throws `TypeError`. Cleanup uses `try { delete w[key] } catch { w[key] = undefined }` for each global. The `useEffect` also removes any pending `'load'` listener on an already-present script tag to prevent orphaned callbacks on unmount.
